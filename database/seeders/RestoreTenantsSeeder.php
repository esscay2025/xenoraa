        ...
        if ($existingBlogCount < 3) {
            ...
            $posts = [
                [
                    ...
                    'summary'      => 'A deep dive into how I architected Xenoraa — a multi-tenant SaaS platform using Laravel, with tenant isolation, custom domains, and modular features.',
                ...
                [
                    ...
                    'summary'      => 'A practical comparison of the three major cloud providers from the perspective of a startup CTO who has worked with all three.',
    ...
    private function bootstrapTenant(int $tid, string $username, string $template): void
    {
        ...
        foreach ($def['jobs'] as $job) {
            DB::table('jobs')->insert([
                ...
                'salary_range' => $job['salary_range'] ?? (isset($job['salary_min']) ? '₹' . number_format($job['salary_min']) . ' – ₹' . number_format($job['salary_max']) : null),
                'status'       => 'active',
                'created_at'   => now(),
                'updated_at'   => now(),
            ]);
