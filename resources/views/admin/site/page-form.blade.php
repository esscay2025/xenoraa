            ...
            @if(isset($page) && $page && $page->page_type && isset($sections) && count($sections))
                <div class="pf-card">
                    ...
                    <div class="info-banner">
                        ...
                        <span>Toggle sections on or off to control what appears on this page. Expand a section to customise its heading and content. Click <strong>Save Page</strong> to apply.</span>
                    </div>
                    @foreach($sections as $section)
                        @php $key=$section['key']; $enabled=$section['enabled']??false; $data=$section['data']??[]; @endphp
                        <div class="section-card {{ $enabled?'enabled':'' }}" id="sc-{{ $key }}">
                            ...
                            <div class="section-body" id="body-{{ $key }}">
                                ...
                                <div class="section-field"><label>Section Heading</label>
                                    <input type="text" name="sections[{{ $key }}][data][heading]" value="{{ old("sections.{$key}.data.heading",$data['heading']??'') }}" placeholder="Leave blank to use default"></div>
                                @endif
                                @if(array_key_exists('subheading',$data))
                                <div class="section-field"><label>Sub-heading / Tagline</label>
                                    <input type="text" name="sections[{{ $key }}][data][subheading]" value="{{ old("sections.{$key}.data.subheading",$data['subheading']??'') }}" placeholder="Optional sub-heading"></div>
                                @endif
                                @if(array_key_exists('text',$data))
                                <div class="section-field"><label>Body Text</label>
                                    <textarea name="sections[{{ $key }}][data][text]" rows="3" placeholder="Optional body text">{{ old("sections.{$key}.data.text",$data['text']??'') }}</textarea></div>
                                @endif
                                @if(array_key_exists('image',$data))
                                <div class="section-field"><label>Image URL</label>
                                    <input type="text" name="sections[{{ $key }}][data][image]" value="{{ old("sections.{$key}.data.image",$data['image']??'') }}" placeholder="https://... or /storage/..."></div>
                                @endif
                                @if(array_key_exists('cta_text',$data))
                                <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.75rem;">
                                    <div class="section-field"><label>CTA Button Text</label>
                                        <input type="text" name="sections[{{ $key }}][data][cta_text]" value="{{ old("sections.{$key}.data.cta_text",$data['cta_text']??'') }}" placeholder="Get Started"></div>
                                    <div class="section-field"><label>CTA Button URL</label>
                                        <input type="text" name="sections[{{ $key }}][data][cta_url]" value="{{ old("sections.{$key}.data.cta_url",$data['cta_url']??'') }}" placeholder="/contact"></div>
                                </div>
                                @endif
                                @if(array_key_exists('button_text',$data))
                                <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.75rem;">
                                    <div class="section-field"><label>Button Text</label>
                                        <input type="text" name="sections[{{ $key }}][data][button_text]" value="{{ old("sections.{$key}.data.button_text",$data['button_text']??'') }}" placeholder="Contact Me"></div>
                                    <div class="section-field"><label>Button URL</label>
                                        <input type="text" name="sections[{{ $key }}][data][button_url]" value="{{ old("sections.{$key}.data.button_url",$data['button_url']??'') }}" placeholder="/contact"></div>
                                </div>
                                @endif
                                @if(array_key_exists('count',$data))
                                <div class="section-field"><label>Number of items to show</label>
                                    <input type="number" name="sections[{{ $key }}][data][count]" min="1" max="20" value="{{ old("sections.{$key}.data.count",$data['count']??3) }}"></div>
                                @endif
                                @if(array_key_exists('posts_per_page',$data))
                                <div class="section-field"><label>Posts per page</label>
                                    <input type="number" name="sections[{{ $key }}][data][posts_per_page]" min="3" max="50" value="{{ old("sections.{$key}.data.posts_per_page",$data['posts_per_page']??9) }}"></div>
                                @endif
                                @if(array_key_exists('products_per_page',$data))
                                <div class="section-field"><label>Products per page</label>
                                    <input type="number" name="sections[{{ $key }}][data][products_per_page]" min="4" max="48" value="{{ old("sections.{$key}.data.products_per_page",$data['products_per_page']??12) }}"></div>
                                @endif
                                @if(array_key_exists('bg_color',$data))
                                <div class="section-field"><label>Background Colour</label>
                                    <input type="text" name="sections[{{ $key }}][data][bg_color]" value="{{ old("sections.{$key}.data.bg_color",$data['bg_color']??'') }}" placeholder="#ffffff"></div>
                                @endif
                                @if(array_key_exists('embed_url',$data))
                                <div class="section-field"><label>Google Maps Embed URL</label>
                                    <input type="text" name="sections[{{ $key }}][data][embed_url]" value="{{ old("sections.{$key}.data.embed_url",$data['embed_url']??'') }}" placeholder="https://www.google.com/maps/embed?..."></div>
                                @endif
                                @if(array_key_exists('email',$data))
                                <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.75rem;">
                                    <div class="section-field"><label>Contact Email</label>
                                        <input type="email" name="sections[{{ $key }}][data][email]" value="{{ old("sections.{$key}.data.email",$data['email']??'') }}" placeholder="hello@example.com"></div>
                                    <div class="section-field"><label>Phone</label>
                                        <input type="text" name="sections[{{ $key }}][data][phone]" value="{{ old("sections.{$key}.data.phone",$data['phone']??'') }}" placeholder="+91 ..."></div>
                                </div>
                                <div class="section-field"><label>Address</label>
                                    <input type="text" name="sections[{{ $key }}][data][address]" value="{{ old("sections.{$key}.data.address",$data['address']??'') }}" placeholder="City, Country"></div>
                                @endif
...
<script>
const quill=new Quill('#editor-container',{theme:'snow',placeholder:'Write your page content here...',modules:{toolbar:[[{'header':[1,2,3,4,false]}],['bold','italic','underline','strike'],[{'color':[]},{'background':[]}],[{'list':'ordered'},{'list':'bullet'}],[{'indent':'-1'},{'indent':'+1'}],['blockquote','code-block'],['link','image'],['clean']]}});
...
document.getElementById('pageForm').addEventListener('submit',function(){document.getElementById('pageContent').value=quill.root.innerHTML;});
</script>
@endif
<script>
...
if(titleInput&&slugInput&&!slugInput.readOnly){
    titleInput.addEventListener('input',function(){if(!slugManuallyEdited)slugInput.value=this.value.toLowerCase().replace(/[^a-z0-9\s-]/g,'').trim().replace(/\s+/g,'-');});
    slugInput.addEventListener('input',function(){slugManuallyEdited=true;});
}
function toggleSection(key){const body=document.getElementById('body-'+key);const chevron=document.getElementById('chevron-'+key);if(!body)return;const isOpen=body.classList.contains('open');body.classList.toggle('open',!isOpen);if(chevron)chevron.style.transform=isOpen?'':'rotate(180deg)';}
function onToggleChange(key,checked){const card=document.getElementById('sc-'+key);const badge=document.getElementById('badge-'+key);if(card)card.classList.toggle('enabled',checked);if(badge){badge.textContent=checked?'ON':'OFF';badge.className='section-badge '+(checked?'on':'off');}}
document.addEventListener('DOMContentLoaded',function(){document.querySelectorAll('.section-toggle input:checked').forEach(function(cb){const key=cb.id.replace('toggle-','');const body=document.getElementById('body-'+key);if(body)body.classList.add('open');const chevron=document.getElementById('chevron-'+key);if(chevron)chevron.style.transform='rotate(180deg)';});});
</script>
@endsection
