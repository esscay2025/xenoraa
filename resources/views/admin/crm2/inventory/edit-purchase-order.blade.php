...
<div class="cf-page">
    <div class="cf-header">
        <a href="{{ route('admin.crm2.inventory.purchase-orders') }}" class="cf-back">&#8592; Purchase Orders</a>
        <h1>Edit Purchase Order</h1>
    </div>
    <form method="POST" action="{{ route('admin.crm2.inventory.update', ['type'=>'purchase_order','id'=>$item->id]) }}" onsubmit="serializeLineItems('po_items','po_items_json')">
        @csrf @method('PATCH')

        <div class="cf-section">
            <div class="cf-section-header" onclick="toggleSection(this)">
                <h3>Purchase Order Information</h3><span class="cf-chevron">&#9660;</span>
            </div>
            <div class="cf-section-body">
                <div class="cf-grid cf-grid-3">
                    <div class="cf-field"><label>PO Owner</label>
                    ...
                    <div class="cf-field"><label>Tracking Number</label><input type="text" name="tracking_number" value="{{ old('tracking_number', $item->tracking_number) }}"></div>
                    <div class="cf-field"><label>PO Date</label><input type="date" name="po_date" value="{{ old('po_date', $item->po_date ? substr($item->po_date,0,10) : '') }}"></div>
                    ...
                    <div class="cf-field"><label>Sales Order Ref.</label><input type="text" name="sales_order_ref" value="{{ old('sales_order_ref', $item->sales_order_ref) }}"></div>
                    <div class="cf-field"><label>Requisition No.</label><input type="text" name="requisition_no" value="{{ old('requisition_no', $item->requisition_no) }}"></div>
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:2rem">
                    <div>
                        ...
                        <div class="cf-grid" style="grid-template-columns:1fr 1fr">
                            <div class="cf-field cf-field-full"><label>Country</label><input type="text" name="bill_country" value="{{ old('bill_country', $item->bill_country) }}"></div>
                            <div class="cf-field cf-field-full"><label>Building</label><input type="text" name="bill_building" value="{{ old('bill_building', $item->bill_building) }}"></div>
                            <div class="cf-field cf-field-full"><label>Street</label><input type="text" name="bill_street" value="{{ old('bill_street', $item->bill_street) }}"></div>
                            <div class="cf-field"><label>City</label><input type="text" name="bill_city" value="{{ old('bill_city', $item->bill_city) }}"></div>
                            <div class="cf-field"><label>State</label><input type="text" name="bill_state" value="{{ old('bill_state', $item->bill_state) }}"></div>
                            <div class="cf-field"><label>Zip</label><input type="text" name="bill_zip" value="{{ old('bill_zip', $item->bill_zip) }}"></div>
                        </div>
                    </div>
                    <div>
                        <h4 style="color:var(--cf-accent);margin:0 0 .75rem;font-size:.85rem">Shipping Address</h4>
                        <div class="cf-grid" style="grid-template-columns:1fr 1fr">
                            <div class="cf-field cf-field-full"><label>Country</label><input type="text" name="ship_country" value="{{ old('ship_country', $item->ship_country) }}"></div>
                            <div class="cf-field cf-field-full"><label>Building</label><input type="text" name="ship_building" value="{{ old('ship_building', $item->ship_building) }}"></div>
                            <div class="cf-field cf-field-full"><label>Street</label><input type="text" name="ship_street" value="{{ old('ship_street', $item->ship_street) }}"></div>
                            <div class="cf-field"><label>City</label><input type="text" name="ship_city" value="{{ old('ship_city', $item->ship_city) }}"></div>
                            <div class="cf-field"><label>State</label><input type="text" name="ship_state" value="{{ old('ship_state', $item->ship_state) }}"></div>
                            <div class="cf-field"><label>Zip</label><input type="text" name="ship_zip" value="{{ old('ship_zip', $item->ship_zip) }}"></div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="cf-section">
            ...
            <div class="cf-section-body">
                <input type="hidden" name="line_items" id="po_items_json">
                <table class="li-table" id="po_items">
                    <thead><tr><th>#</th><th>Product Name</th><th>Qty</th><th>Unit Price (₹)</th><th>Amount (₹)</th><th>Discount (₹)</th><th>Tax (₹)</th><th>Total (₹)</th><th></th></tr></thead>
                    <tbody>
                        ...
                        @if(!empty($lineItems))
                            ...
                            <tr>
                                ...
                                <td><input type="number" class="li-qty" value="{{ $li['qty'] ?? 1 }}" min="1" style="width:60px" oninput="recalcTotals('po_items')"></td>
                                <td><input type="number" class="li-price" step="0.01" value="{{ $li['price'] ?? 0 }}" style="width:90px" oninput="recalcTotals('po_items')"></td>
                                <td><input type="number" class="li-amt" step="0.01" value="{{ $li['amount'] ?? 0 }}" style="width:90px" readonly></td>
                                <td><input type="number" class="li-disc" step="0.01" value="{{ $li['discount'] ?? 0 }}" style="width:80px" oninput="recalcTotals('po_items')"></td>
                                <td><input type="number" class="li-tax" step="0.01" value="{{ $li['tax'] ?? 0 }}" style="width:80px" oninput="recalcTotals('po_items')"></td>
                                <td><input type="number" class="li-total" step="0.01" value="{{ $li['total'] ?? 0 }}" style="width:90px" readonly></td>
                                <td><button type="button" class="li-remove-btn" onclick="removeLineItem(this,'po_items')">&#10005;</button></td>
                            </tr>
                            @endforeach
                        @else
                        <tr><td>1</td><td><input type="text" class="li-product"></td><td><input type="number" class="li-qty" value="1" style="width:60px" oninput="recalcTotals('po_items')"></td><td><input type="number" class="li-price" step="0.01" value="0" style="width:90px" oninput="recalcTotals('po_items')"></td><td><input type="number" class="li-amt" step="0.01" value="0" style="width:90px" readonly></td><td><input type="number" class="li-disc" step="0.01" value="0" style="width:80px" oninput="recalcTotals('po_items')"></td><td><input type="number" class="li-tax" step="0.01" value="0" style="width:80px" oninput="recalcTotals('po_items')"></td><td><input type="number" class="li-total" step="0.01" value="0" style="width:90px" readonly></td><td><button type="button" class="li-remove-btn" onclick="removeLineItem(this,'po_items')">&#10005;</button></td></tr>
                        @endif
                    </tbody>
                </table>
                <button type="button" class="li-add-btn" onclick="addLineItemWithSearch('po_items')">+ Add Row</button>
                <div class="li-summary">
                    <div class="li-summary-row"><label>Sub Total (₹)</label><input type="number" id="po_items_subtotal" name="subtotal" step="0.01" value="{{ $item->subtotal ?? 0 }}" readonly></div>
                    <div class="li-summary-row"><label>Discount (₹)</label><input type="number" id="po_items_discount" name="discount_amount" step="0.01" value="{{ $item->discount_amount ?? 0 }}" oninput="recalcTotals('po_items')"></div>
                    <div class="li-summary-row"><label>Tax (₹)</label><input type="number" id="po_items_tax" name="tax_amount" step="0.01" value="{{ $item->tax_amount ?? 0 }}" oninput="recalcTotals('po_items')"></div>
                    <div class="li-summary-row"><label>Adjustment (₹)</label><input type="number" id="po_items_adjustment" name="adjustment" step="0.01" value="{{ $item->adjustment ?? 0 }}" oninput="recalcTotals('po_items')"></div>
                    <div class="li-summary-row li-grand-total"><label>Grand Total (₹)</label><input type="number" id="po_items_grand" name="grand_total" step="0.01" value="{{ $item->grand_total ?? 0 }}" readonly></div>
                </div>
            </div>
        </div>
        ...
        <div class="cf-actions">
            <button type="submit" class="cf-btn cf-btn-primary">&#10003; Update Purchase Order</button>
            <a href="{{ route('admin.crm2.inventory.purchase-orders') }}" class="cf-btn cf-btn-secondary">Cancel</a>
        </div>
    </form>
</div>
@endsection
