...
<div class="cv-page">
    <div class="cv-header">
        ...
        <div class="cv-actions">
            <a href="{{ route('admin.crm2.inventory.price-books.edit', $item->id) }}" class="cf-btn cf-btn-primary">&#9998; Edit</a>
            <a href="{{ route('admin.crm2.inventory.price-books') }}" class="cf-btn cf-btn-secondary">&#8592; Back</a>
            <form method="POST" action="{{ route('admin.crm2.inventory.destroy', ['type'=>'price_book','id'=>$item->id]) }}" style="display:inline" onsubmit="return confirm('Delete this price book?')">
                @csrf @method('DELETE')
    ...
    <div class="cv-section">
        <div class="cv-section-header"><h3>Price Book Information</h3></div>
        <div class="cv-section-body">
            <div class="cv-grid">
                <div class="cv-field"><span class="cv-label">Price Book Name</span><span class="cv-value">{{ $item->name ?: '—' }}</span></div>
                <div class="cv-field"><span class="cv-label">Pricing Model</span><span class="cv-value">{{ $item->pricing_model ?: '—' }}</span></div>
                <div class="cv-field"><span class="cv-label">Pricing Percentage</span><span class="cv-value">{{ $item->pricing_percentage ? $item->pricing_percentage . '%' : '—' }}</span></div>
                ...
                <div class="cv-field"><span class="cv-label">Owner</span><span class="cv-value">{{ $item->owner ? $item->owner->name : '—' }}</span></div>
                <div class="cv-field"><span class="cv-label">Created</span><span class="cv-value">{{ $item->created_at ? $item->created_at->format('d M Y') : '—' }}</span></div>
            </div>
        </div>
    </div>
