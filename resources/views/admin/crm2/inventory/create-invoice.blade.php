    ...
    <form method="POST" action="{{ route('admin.crm2.inventory.store') }}" onsubmit="serializeLineItems('inv_items','inv_items_json')">
        ...
        <div class="cf-section">
            <div class="cf-section-header" onclick="toggleSection(this)">
                <h3>Invoice Information</h3><span class="cf-chevron">&#9660;</span>
            </div>
            <div class="cf-section-body">
                <div class="cf-grid cf-grid-3">
                    <div class="cf-field"><label>Invoice Owner</label>
                    ...
                    <div class="cf-field"><label>Subject *</label>
                        <input type="text" name="subject" required value="{{ old('subject') }}" placeholder="Invoice subject"></div>
                    <div class="cf-field"><label>Status</label>
                        <select name="status">
                        @foreach(['Draft','Sent','Paid','Void','Overdue'] as $s)
                        <option value="{{ $s }}">{{ $s }}</option>@endforeach</select>
                    </div>
                    <div class="cf-field"><label>Invoice Date</label>
                        <input type="date" name="invoice_date" value="{{ old('invoice_date', date('Y-m-d')) }}"></div>
                    <div class="cf-field"><label>Due Date</label>
                        <input type="date" name="due_date" value="{{ old('due_date') }}"></div>
                    ...
                    <div class="cf-field"><label>Payment Terms</label>
                        ...
                        <option value="{{ $t }}">{{ $t }}</option>@endforeach</select>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="cf-section">
            ...
            <div class="cf-section-body">
                <input type="hidden" name="line_items" id="inv_items_json">
                <table class="li-table" id="inv_items">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td><input type="number" class="li-qty" value="1" min="1" style="width:60px" oninput="recalcTotals('inv_items')"></td>
                            <td><input type="number" class="li-price" step="0.01" value="0" style="width:90px" oninput="recalcTotals('inv_items')"></td>
                            <td><input type="number" class="li-amt" step="0.01" value="0" style="width:90px" readonly></td>
                            <td><input type="number" class="li-disc" step="0.01" value="0" style="width:80px" oninput="recalcTotals('inv_items')"></td>
                            <td><input type="number" class="li-tax" step="0.01" value="0" style="width:80px" oninput="recalcTotals('inv_items')"></td>
                            <td><input type="number" class="li-total" step="0.01" value="0" style="width:90px" readonly></td>
                            <td><button type="button" class="li-remove-btn" onclick="removeLineItem(this,'inv_items')">&#10005;</button></td>
                        </tr>
                    </tbody>
                </table>
                <button type="button" class="li-add-btn" onclick="addLineItemWithSearch('inv_items')">+ Add Row</button>
                <div class="li-summary">
                    <div class="li-summary-row"><label>Sub Total (₹)</label><input type="number" id="inv_items_subtotal" name="subtotal" step="0.01" value="0" readonly></div>
                    <div class="li-summary-row"><label>Discount (₹)</label><input type="number" id="inv_items_discount" name="discount_amount" step="0.01" value="0" oninput="recalcTotals('inv_items')"></div>
                    <div class="li-summary-row"><label>Tax (₹)</label><input type="number" id="inv_items_tax" name="tax_amount" step="0.01" value="0" oninput="recalcTotals('inv_items')"></div>
                    <div class="li-summary-row"><label>Adjustment (₹)</label><input type="number" id="inv_items_adjustment" name="adjustment" step="0.01" value="0" oninput="recalcTotals('inv_items')"></div>
                    <div class="li-summary-row li-grand-total"><label>Grand Total (₹)</label><input type="number" id="inv_items_grand" name="grand_total" step="0.01" value="0" readonly></div>
                </div>
            </div>
        </div>
