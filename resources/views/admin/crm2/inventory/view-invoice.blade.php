...
<div class="cv-page">
    <div class="cv-header">
        <div class="cv-title-block">
            <h1>{{ $item->subject ?: 'Invoice #' . $item->id }}</h1>
            <span class="cv-badge">{{ $item->status ?: 'Draft' }}</span>
        </div>
        <div class="cv-actions">
            <a href="{{ route('admin.crm2.inventory.invoices.edit', $item->id) }}" class="cf-btn cf-btn-primary">&#9998; Edit</a>
            <a href="{{ route('admin.crm2.inventory.invoices') }}" class="cf-btn cf-btn-secondary">&#8592; Back</a>
            <form method="POST" action="{{ route('admin.crm2.inventory.destroy', ['type'=>'invoice','id'=>$item->id]) }}" style="display:inline" onsubmit="return confirm('Delete?')">
                @csrf @method('DELETE')
    ...
    <div class="cv-section">
        <div class="cv-section-header"><h3>Invoice Information</h3></div>
        <div class="cv-section-body">
            <div class="cv-grid">
                ...
                <div class="cv-field"><span class="cv-label">Invoice Date</span><span class="cv-value">{{ $item->invoice_date ? \Carbon\Carbon::parse($item->invoice_date)->format('d M Y') : '—' }}</span></div>
                <div class="cv-field"><span class="cv-label">Due Date</span><span class="cv-value">{{ $item->due_date ? \Carbon\Carbon::parse($item->due_date)->format('d M Y') : '—' }}</span></div>
    ...
    <div class="cv-section">
        <div class="cv-section-header"><h3>Invoice Items</h3></div>
        <div class="cv-section-body">
...
<div class="send-mail-panel no-print" id="sendMailPanel">
    ...
    <div class="send-mail-body">
        <div class="send-mail-attach-note">
            &#128206; A PDF copy of this Invoice will be automatically attached to the email.
        </div>
        <form method="POST" action="{{ route('admin.crm2.inventory.send-mail', ['type'=>'invoice','id'=>$item->id]) }}">
            @csrf
            ...
            <div class="send-mail-field">
                ...
                <input type="text" name="subject" required
                       value="{{ $item->subject ?: 'Invoice #' . $item->id }}">
            </div>
            <div class="send-mail-field">
                <label>Message</label>
