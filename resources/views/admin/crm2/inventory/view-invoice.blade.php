  ...
  <div class="inv-main">
    ...
    <div class="inv-header">
      ...
      <div style="position:relative;display:inline-block;">
        <button id="invActBtn" style="display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:7px;background:var(--bg-card);border:1.5px solid var(--border);color:var(--text-secondary);font-size:1.15rem;cursor:pointer;transition:all .15s;" title="More actions">&#8942;</button>
        <div id="invActDrop" style="display:none;position:absolute;right:0;top:calc(100% + 6px);background:var(--bg-card);border:1px solid var(--border);border-radius:8px;min-width:200px;box-shadow:0 8px 24px rgba(0,0,0,0.35);z-index:999;overflow:hidden;">
          <a href="{{ route('admin.crm2.inventory.invoices.clone', $item->id) }}" style="display:flex;align-items:center;gap:.75rem;padding:.65rem 1rem;font-size:.82rem;color:var(--text-primary);text-decoration:none;" onmouseover="this.style.background='var(--bg-hover)'" onmouseout="this.style.background='transparent'">
            <i class="fas fa-copy" style="width:16px;color:#6366f1;"></i> Clone
          </a>
          <a href="{{ route('admin.crm2.inventory.invoices.export-pdf', $item->id) }}" style="display:flex;align-items:center;gap:.75rem;padding:.65rem 1rem;font-size:.82rem;color:var(--text-primary);text-decoration:none;" onmouseover="this.style.background='var(--bg-hover)'" onmouseout="this.style.background='transparent'">
          ...
          <button onclick="document.getElementById('invSendMailSlider').classList.add('open');document.getElementById('invSendMailOverlay').classList.add('open');" style="display:flex;align-items:center;gap:.75rem;padding:.65rem 1rem;font-size:.82rem;color:var(--text-primary);background:transparent;border:none;cursor:pointer;width:100%;text-align:left;" onmouseover="this.style.background='var(--bg-hover)'" onmouseout="this.style.background='transparent'">
            <i class="fas fa-envelope" style="width:16px;color:#3b82f6;"></i> Send Mail
          </button>
          <hr style="border:none;border-top:1px solid var(--border);margin:0;">
          <form method="POST" action="{{ route('admin.crm2.inventory.invoices.destroy', $item->id) }}" onsubmit="return confirm('Delete this invoice? This cannot be undone.')">
            @csrf @method('DELETE')
...
// 3-dot Actions Menu
document.getElementById('invActBtn')?.addEventListener('click', function(e) {
    e.stopPropagation();
    const drop = document.getElementById('invActDrop');
    drop.style.display = drop.style.display === 'block' ? 'none' : 'block';
});
document.addEventListener('click', function(e) {
    const btn = document.getElementById('invActBtn');
    const drop = document.getElementById('invActDrop');
