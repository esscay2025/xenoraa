...
<div class="cf-page">
    ...
    <form method="POST" action="{{ route('admin.crm2.inventory.update', ['type'=>'invoice','id'=>$item->id]) }}" onsubmit="serializeLineItems('inv_items','inv_items_json')">
        @csrf @method('PATCH')
            ...
            <div class="cf-section-body">
                <div class="cf-grid cf-grid-3">
                    ...
                    <div class="cf-field"><label>Status</label>
                        <select name="status">@foreach(['Draft','Sent','Paid','Void','Overdue'] as $s)
                        <option value="{{ $s }}" {{ $item->status == $s ? 'selected' : '' }}>{{ $s }}</option>@endforeach</select>
                    </div>
                    <div class="cf-field"><label>Invoice Date</label><input type="date" name="invoice_date" value="{{ old('invoice_date', $item->invoice_date ? substr($item->invoice_date,0,10) : '') }}"></div>
                    <div class="cf-field"><label>Due Date</label><input type="date" name="due_date" value="{{ old('due_date', $item->due_date ? substr($item->due_date,0,10) : '') }}"></div>
        ...
        <div class="cf-section">
            ...
            <div class="cf-section-body">
                ...
                <div class="li-summary">
                    <div class="li-summary-row"><label>Sub Total (₹)</label><input type="number" id="inv_items_subtotal" name="subtotal" step="0.01" value="{{ $item->subtotal ?? 0 }}" readonly></div>
                    <div class="li-summary-row"><label>Discount (₹)</label><input type="number" id="inv_items_discount" name="discount_amount" step="0.01" value="{{ $item->discount_amount ?? 0 }}" oninput="recalcTotals('inv_items')"></div>
                    <div class="li-summary-row"><label>Tax (₹)</label><input type="number" id="inv_items_tax" name="tax_amount" step="0.01" value="{{ $item->tax_amount ?? 0 }}" oninput="recalcTotals('inv_items')"></div>
                    <div class="li-summary-row"><label>Adjustment (₹)</label><input type="number" id="inv_items_adjustment" name="adjustment" step="0.01" value="{{ $item->adjustment ?? 0 }}" oninput="recalcTotals('inv_items')"></div>
                    <div class="li-summary-row li-grand-total"><label>Grand Total (₹)</label><input type="number" id="inv_items_grand" name="grand_total" step="0.01" value="{{ $item->grand_total ?? 0 }}" readonly></div>
                </div>
            </div>
        </div>
        ...
        <div class="cf-actions">
            <button type="submit" class="cf-btn cf-btn-primary">&#10003; Update Invoice</button>
