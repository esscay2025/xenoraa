...
<div class="cf-page">
    <div class="cf-header">
        ...
        <h1>Edit Price Book</h1>
    </div>
    <form method="POST" action="{{ route('admin.crm2.inventory.update', ['type'=>'price_book','id'=>$item->id]) }}">
        @csrf @method('PATCH')
                ...
                <div class="cf-grid cf-grid-3">
                    ...
                    <div class="cf-field">
                        ...
                        <select name="pricing_model">
                            ...
                            @foreach(['flat'=>'Flat Rate','percentage'=>'Percentage','tiered'=>'Tiered','volume'=>'Volume'] as $v=>$l)
                            <option value="{{ $v }}" {{ $item->pricing_model == $v ? 'selected' : '' }}>{{ $l }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="cf-field">
                        <label>Pricing Percentage (%)</label>
                        <input type="number" name="pricing_percentage" step="0.01" value="{{ old('pricing_percentage', $item->pricing_percentage) }}">
                    </div>
                    <div class="cf-field">
                        <label>Currency</label>
                        <select name="currency">
                            @foreach(['INR'=>'INR (₹)','USD'=>'USD ($)','EUR'=>'EUR (€)','GBP'=>'GBP (£)'] as $v=>$l)
                            <option value="{{ $v }}" {{ ($item->currency ?? 'INR') == $v ? 'selected' : '' }}>{{ $l }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="cf-field">
        ...
        <div class="cf-actions">
            <button type="submit" class="cf-btn cf-btn-primary">&#10003; Update Price Book</button>
