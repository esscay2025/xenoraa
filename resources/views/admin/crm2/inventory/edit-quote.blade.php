  ...
  <div class="crm2-card"><div class="crm2-card-body">
    <form method="POST" action="{{ route('admin.crm2.inventory.update', ['type'=>'quotes','id'=>\$item->id]) }}">@csrf @method('PATCH')
      <div class="crm2-form-grid">
        ...
        <div class="form-group"><label>Status</label><select name="status" class="crm2-select"><option value="draft" {{ $item->status=='draft'?'selected':'' }}>Draft</option><option value="sent" {{ $item->status=='sent'?'selected':'' }}>Sent</option><option value="accepted" {{ $item->status=='accepted'?'selected':'' }}>Accepted</option><option value="rejected" {{ $item->status=='rejected'?'selected':'' }}>Rejected</option></select></div>
        <div class="form-group"><label>Valid Until</label><input type="date" name="valid_until" class="crm2-input" value="{{ old('valid_until', $item->valid_until ? $item->valid_until->format('Y-m-d') : '') }}"></div>
