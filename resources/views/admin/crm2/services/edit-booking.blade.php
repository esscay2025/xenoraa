  ...
  <div class="crm2-card"><div class="crm2-card-body">
    <form method="POST" action="{{ route('admin.crm2.services.update', ['type'=>'booking','id'=>$item->id]) }}">@csrf @method('PATCH')
      <div class="crm2-form-grid">
        <div class="form-group"><label>Service *</label><select name="service_id" class="crm2-select" required><option value="">— Select Service —</option>@foreach($services_list as $s)<option value="{{ $s->id }}" {{ $item->service_id==$s->id?'selected':'' }}>{{ $s->name }}</option>@endforeach</select></div>
        <div class="form-group"><label>Booking Date & Time *</label><input type="datetime-local" name="booking_time" class="crm2-input" value="{{ old('booking_time', $item->booking_time ? \Carbon\Carbon::parse($item->booking_time)->format('Y-m-d\TH:i') : '') }}" required></div>
        <div class="form-group"><label>Status</label><select name="status" class="crm2-select"><option value="scheduled" {{ $item->status=='scheduled'?'selected':'' }}>Scheduled</option><option value="confirmed" {{ $item->status=='confirmed'?'selected':'' }}>Confirmed</option><option value="completed" {{ $item->status=='completed'?'selected':'' }}>Completed</option><option value="cancelled" {{ $item->status=='cancelled'?'selected':'' }}>Cancelled</option></select></div>
