...
<div class="crm2-page">
  ...
  <div class="xn-sticky-bar">
    ...
    <div class="xn-sticky-actions">
      <a href="{{ route('admin.crm2.activities.'.($type === 'task' ? 'tasks' : ($type === 'meeting' ? 'meetings' : 'calls'))) }}" class="xn-sticky-btn xn-sticky-btn-ghost">
        <i class="fas fa-arrow-left"></i> Cancel
      </a>
      <button type="submit" form="activityCreateForm" class="xn-sticky-btn xn-sticky-btn-primary">
        <i class="fas fa-save"></i> Save {{ ucfirst($type) }}
      </button>
    </div>
  </div>
  ...
  <div class="crm2-card"><div class="crm2-card-body">
    <form id="activityCreateForm" method="POST" action="{{ route('admin.crm2.activity.store') }}">@csrf
      <input type="hidden" name="type" value="{{ $type }}">
      <div class="crm2-form-grid">
        <div class="form-group full"><label>Subject *</label><input type="text" name="subject" class="crm2-input" required autofocus placeholder="Subject of this {{ $type }}..."></div>
        <div class="form-group"><label>Due Date & Time</label><input type="datetime-local" name="due_at" class="crm2-input"></div>
