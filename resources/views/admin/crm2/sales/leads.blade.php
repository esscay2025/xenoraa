...
<div class="crm2-page">
  ...
  <div class="crm2-card"><div class="crm2-card-body p-0">
    <table class="crm2-table" id="leadsTable">
      <thead>
        <tr>
      ...
      <tbody>
        ...
        @php $viewUrl = route('admin.crm2.sales.leads.show', $lead->id); @endphp
        <tr class="clickable-row" onclick="rowNav(event, '{{ $viewUrl }}')">
          <td class="cb-col" onclick="event.stopPropagation()">
            <input type="checkbox" class="lead-cb" value="{{ $lead->id }}" onchange="updateSelection()">
          </td>
          <td style="font-weight:600;color:var(--accent,#6366f1)">{{ $lead->first_name ? $lead->first_name.' '.$lead->last_name : $lead->name }}</td>
          <td>{{ $lead->email ?? '—' }}</td>
          <td>{{ $lead->company ?? '—' }}</td>
...
<form id="bulkDeleteForm" method="POST" action="{{ route('admin.crm2.sales.leads.bulk-delete') }}" style="display:none">
  @csrf
  @method('DELETE')
...
<form id="bulkTaskForm" method="POST" action="{{ route('admin.crm2.sales.leads.bulk-task') }}" style="display:none">
  @csrf
...
function getSelectedIds() {
  return Array.from(document.querySelectorAll('.lead-cb:checked')).map(c => c.value);
}
...
function bulkExport() {
  const ids = getSelectedIds();
  if (!ids.length) { alert('Please select at least one lead to export.'); return; }
  const rows = [['Name','Email','Company','Source','Status','Value','Created']];
  document.querySelectorAll('#leadsTable tbody tr.clickable-row').forEach(tr => {
