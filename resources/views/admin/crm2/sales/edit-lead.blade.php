  ...
  <div class="crm2-card"><div class="crm2-card-body">
    <form method="POST" action="{{ route('admin.crm2.sales.update', ['type'=>'lead','id'=>\$item->id]) }}">@csrf @method('PATCH')
      <div class="crm2-form-grid">
        ...
        <div class="form-group"><label>Company</label><input type="text" name="company" class="crm2-input" value="{{ old('company', $item->company) }}"></div>
        <div class="form-group"><label>Status</label><select name="status" class="crm2-select"><option value="new" {{ $item->status=='new'?'selected':'' }}>New</option><option value="contacted" {{ $item->status=='contacted'?'selected':'' }}>Contacted</option><option value="qualified" {{ $item->status=='qualified'?'selected':'' }}>Qualified</option><option value="unqualified" {{ $item->status=='unqualified'?'selected':'' }}>Unqualified</option><option value="converted" {{ $item->status=='converted'?'selected':'' }}>Converted</option></select></div>
        <div class="form-group"><label>Source</label><select name="source" class="crm2-select"><option value="">— None —</option><option value="website" {{ $item->source=='website'?'selected':'' }}>Website</option><option value="referral" {{ $item->source=='referral'?'selected':'' }}>Referral</option><option value="social_media" {{ $item->source=='social_media'?'selected':'' }}>Social Media</option><option value="cold_call" {{ $item->source=='cold_call'?'selected':'' }}>Cold Call</option><option value="email" {{ $item->source=='email'?'selected':'' }}>Email</option><option value="other" {{ $item->source=='other'?'selected':'' }}>Other</option></select></div>
