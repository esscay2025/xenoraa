...
.cf-section-body.cols-2{grid-template-columns:repeat(2,1fr)}
...
<div class="cf-wrap">
  ...
  <div class="xn-sticky-bar">
    ...
    <div class="xn-sticky-actions">
      <a href="{{ route('admin.crm2.sales.contacts') }}" class="xn-sticky-btn xn-sticky-btn-ghost">
        <i class="fas fa-arrow-left"></i> Cancel
      </a>
      <button type="submit" form="contactCreateForm" class="xn-sticky-btn xn-sticky-btn-primary">
  ...
  <div class="cf-header">
    <div>
      <div class="cf-breadcrumb"><a href="{{ route('admin.crm2.sales.contacts') }}">Contacts</a> / New Contact</div>
      <h1><i class="fas fa-user-plus"></i> New Contact</h1>
    </div>
  ...
  <form id="contactCreateForm" method="POST" action="{{ route('admin.crm2.sales.contacts.store') }}" enctype="multipart/form-data">
    @csrf
    {{-- Contact Profile --}}
    <div class="cf-section">
      ...
      <div class="cf-section-body">
        <div class="cf-avatar-upload">
          <div class="cf-avatar-preview" id="avatar-preview"><i class="fas fa-user"></i></div>
          <div>
            <label class="cf-btn cf-btn-ghost" style="cursor:pointer"><i class="fas fa-upload"></i> Upload Photo<input type="file" name="contact_image" accept="image/*" style="display:none" onchange="previewAvatar(this)"></label>
        ...
        <div class="cf-field">
          ...
          <select name="salutation">
            <option value="">--</option>
            @foreach(['Mr.','Mrs.','Ms.','Dr.','Prof.'] as $sal)<option value="{{ $sal }}" {{ old('salutation')==$sal?'selected':'' }}>{{ $sal }}</option>@endforeach
          </select>
        </div>
        <div class="cf-field">
        ...
        <div class="cf-field">
          ...
          <select name="reporting_to">
            ...
            @foreach($staff as $s)<option value="{{ $s->id }}" {{ old('reporting_to')==$s->id?'selected':'' }}>{{ $s->name }}</option>@endforeach
          </select>
        </div>
      </div>
    </div>
      ...
      <div class="cf-section-body">
        <div class="cf-field">
          ...
          <select name="account_id">
            <option value="">-- None --</option>
            @foreach($accounts_list as $acc)<option value="{{ $acc->id }}" {{ old('account_id')==$acc->id?'selected':'' }}>{{ $acc->name }}</option>@endforeach
          </select>
        </div>
        <div class="cf-field">
          <label>Vendor Name</label>
          <input type="text" name="vendor_name" value="{{ old('vendor_name') }}" placeholder="Vendor name">
        </div>
        <div class="cf-field">
    ...
    <div class="cf-section">
      ...
      <div class="cf-section-body">
        <div class="cf-field"><label>Email</label><input type="email" name="email" value="{{ old('email') }}" placeholder="email@example.com"></div>
        <div class="cf-field"><label>Secondary Email</label><input type="email" name="secondary_email" value="{{ old('secondary_email') }}" placeholder="secondary@example.com"></div>
        <div class="cf-field"><label>Phone</label><input type="text" name="phone" value="{{ old('phone') }}" placeholder="+1 234 567 8900"></div>
        <div class="cf-field"><label>Mobile</label><input type="text" name="mobile" value="{{ old('mobile') }}" placeholder="+1 234 567 8901"></div>
        <div class="cf-field"><label>Other Phone</label><input type="text" name="other_phone" value="{{ old('other_phone') }}" placeholder="Other phone"></div>
        <div class="cf-field"><label>Home Phone</label><input type="text" name="home_phone" value="{{ old('home_phone') }}" placeholder="Home phone"></div>
        ...
        <div class="cf-field">
          ...
          <div class="cf-check"><input type="checkbox" name="email_opt_out" value="1" {{ old('email_opt_out')?'checked':'' }}><span style="font-size:.85rem;color:var(--text-secondary)">Do not send marketing emails</span></div>
        </div>
      </div>
    </div>
    ...
    <div class="cf-section">
      ...
      <div class="cf-section-body">
        ...
        <div class="cf-field"><label>Assistant</label><input type="text" name="assistant" value="{{ old('assistant') }}" placeholder="Assistant name"></div>
        <div class="cf-field"><label>Assistant Phone</label><input type="text" name="assistant_phone" value="{{ old('assistant_phone') }}" placeholder="Assistant phone"></div>
      </div>
    </div>
    ...
    <div class="cf-section">
      ...
      <div class="cf-section-body">
        <div class="cf-field"><label>Date of Birth</label><input type="date" name="date_of_birth" value="{{ old('date_of_birth') }}"></div>
        <div class="cf-field"><label>Skype ID</label><input type="text" name="skype_id" value="{{ old('skype_id') }}" placeholder="Skype username"></div>
        <div class="cf-field"><label>Twitter / X</label><input type="text" name="twitter" value="{{ old('twitter') }}" placeholder="@handle"></div>
      </div>
    </div>
    {{-- Mailing Address --}}
    <div class="cf-section">
      ...
      <div class="cf-section-body">
        <div class="cf-field"><label>Country</label><input type="text" name="mailing_country" value="{{ old('mailing_country') }}" placeholder="Country"></div>
        <div class="cf-field"><label>Building / Flat</label><input type="text" name="mailing_building" value="{{ old('mailing_building') }}" placeholder="Building name"></div>
        <div class="cf-field"><label>Street</label><input type="text" name="mailing_street" value="{{ old('mailing_street') }}" placeholder="Street address"></div>
        <div class="cf-field"><label>City</label><input type="text" name="mailing_city" value="{{ old('mailing_city') }}" placeholder="City"></div>
        <div class="cf-field"><label>State</label><input type="text" name="mailing_state" value="{{ old('mailing_state') }}" placeholder="State"></div>
        <div class="cf-field"><label>Postal Code</label><input type="text" name="mailing_zip" value="{{ old('mailing_zip') }}" placeholder="ZIP / Postal code"></div>
        <div class="cf-field"><label>Latitude</label><input type="text" name="mailing_lat" value="{{ old('mailing_lat') }}" placeholder="e.g. 13.0827"></div>
        <div class="cf-field"><label>Longitude</label><input type="text" name="mailing_lng" value="{{ old('mailing_lng') }}" placeholder="e.g. 80.2707"></div>
      </div>
    </div>
    {{-- Other Address --}}
    <div class="cf-section">
      ...
      <div class="cf-section-body">
        <div class="cf-field"><label>Country</label><input type="text" name="other_country" value="{{ old('other_country') }}" placeholder="Country"></div>
        <div class="cf-field"><label>Building / Flat</label><input type="text" name="other_building" value="{{ old('other_building') }}" placeholder="Building name"></div>
        <div class="cf-field"><label>Street</label><input type="text" name="other_street" value="{{ old('other_street') }}" placeholder="Street address"></div>
        <div class="cf-field"><label>City</label><input type="text" name="other_city" value="{{ old('other_city') }}" placeholder="City"></div>
        <div class="cf-field"><label>State</label><input type="text" name="other_state" value="{{ old('other_state') }}" placeholder="State"></div>
        <div class="cf-field"><label>Postal Code</label><input type="text" name="other_zip" value="{{ old('other_zip') }}" placeholder="ZIP / Postal code"></div>
        <div class="cf-field"><label>Latitude</label><input type="text" name="other_lat" value="{{ old('other_lat') }}" placeholder="e.g. 13.0827"></div>
        <div class="cf-field"><label>Longitude</label><input type="text" name="other_lng" value="{{ old('other_lng') }}" placeholder="e.g. 80.2707"></div>
      </div>
    </div>
    {{-- Description & Notes --}}
    <div class="cf-section">
      ...
      <div class="cf-section-body" style="grid-template-columns:1fr">
        <div class="cf-field"><label>Description</label><textarea name="description" placeholder="Contact description...">{{ old('description') }}</textarea></div>
    ...
    <div class="cf-actions">
      ...
      <button type="submit" class="cf-btn cf-btn-primary"><i class="fas fa-save"></i> Save Contact</button>
    </div>
  </form>
</div>
<script>
