  ...
  <div class="crm2-card"><div class="crm2-card-body">
    <form method="POST" action="{{ route('admin.crm2.projects.update', ['type'=>'project','id'=>$item->id]) }}">@csrf @method('PATCH')
      <div class="crm2-form-grid">
        ...
        <div class="form-group"><label>Start Date</label><input type="date" name="start_date" class="crm2-input" value="{{ old('start_date', $item->start_date ? \Carbon\Carbon::parse($item->start_date)->format('Y-m-d') : '') }}"></div>
        <div class="form-group"><label>End Date</label><input type="date" name="end_date" class="crm2-input" value="{{ old('end_date', $item->end_date ? \Carbon\Carbon::parse($item->end_date)->format('Y-m-d') : '') }}"></div>
        <div class="form-group"><label>Budget (₹)</label><input type="number" name="budget" class="crm2-input" value="{{ old('budget', $item->budget) }}" step="0.01" min="0"></div>
        <div class="form-group"><label>Cost (₹)</label><input type="number" name="cost" class="crm2-input" value="{{ old('cost', $item->cost) }}" step="0.01" min="0"></div>
