...
.tpl-header{display:flex;align-items:center;gap:1rem;margin-bottom:1.75rem}
...
<div class="tpl-page">
  ...
  <form method="POST" action="{{ route('admin.ecommerce.settings.mail-templates.store') }}" enctype="multipart/form-data" id="tplForm">
    @csrf
      ...
      <div>
          ...
          <div class="tpl-card-body">
            <div class="tpl-grid-2">
              <div class="tpl-field">
                ...
                <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g. Order Confirmation v2" required>
              </div>
              <div class="tpl-field">
            ...
            <div class="tpl-field">
              ...
              <input type="text" name="subject" value="{{ old('subject') }}" placeholder="e.g. Your Order #{{order_number}} is Confirmed!" required>
            </div>
          </div>
        </div>
        ...
        <div class="tpl-card">
          ...
          <div class="tpl-card-body" style="padding-bottom:.5rem">
            ...
            <textarea class="tpl-editor-area" name="body_html" id="bodyHtml" placeholder="Enter your HTML email body here...">{{ old('body_html') }}</textarea>
        ...
        <div class="tpl-card">
          ...
          <div class="tpl-card-body">
            <div class="tpl-field">
              ...
              <small style="font-size:.75rem;color:var(--text-muted)">Max 2MB. PNG or SVG recommended for best quality.</small>
            </div>
      ...
      <div>
          ...
          <div class="tpl-card-body">
            <div class="tpl-field">
              ...
              <div class="tpl-color-row">
                <input type="color" id="primaryColorPicker" value="{{ old('primary_color', '#6366f1') }}" oninput="document.getElementById('primaryColorText').value=this.value">
                <input type="text" name="primary_color" id="primaryColorText" value="{{ old('primary_color', '#6366f1') }}" placeholder="#6366f1" oninput="document.getElementById('primaryColorPicker').value=this.value">
              </div>
            </div>
            <div class="tpl-field">
              <label>Secondary / Background Colour</label>
              <div class="tpl-color-row">
                <input type="color" id="secondaryColorPicker" value="{{ old('secondary_color', '#f8fafc') }}" oninput="document.getElementById('secondaryColorText').value=this.value">
                <input type="text" name="secondary_color" id="secondaryColorText" value="{{ old('secondary_color', '#f8fafc') }}" placeholder="#f8fafc" oninput="document.getElementById('secondaryColorPicker').value=this.value">
              </div>
            </div>
            <div class="tpl-field">
              ...
              <select name="font_family">
                ...
                <option value="{{ $val }}" {{ old('font_family') === $val ? 'selected' : '' }}>{{ $lbl }}</option>
                @endforeach
              </select>
            </div>
          </div>
        ...
        <div class="tpl-card">
          ...
          <div class="tpl-card-body" style="padding:.75rem">
            <iframe class="tpl-preview-frame" id="previewFrame" srcdoc="<p style='color:#94a3b8;text-align:center;padding:2rem;font-family:sans-serif'>Click Preview in the editor toolbar to see your template here.</p>"></iframe>
          </div>
        </div>

        <div class="tpl-actions">
          <button type="submit" class="tpl-btn primary"><i class="fas fa-save"></i> Save Template</button>
...
function insertTag(tag) {
    ...
    const html = `<${tag}>Content here</${tag}>\n`;
...
function insertLink() {
    ...
    const html = `<a href="${url}" style="color:#6366f1">${text}</a>`;
...
function insertDivider() {
    const ta = document.getElementById('bodyHtml');
    const s = ta.selectionStart;
    const html = `<hr style="border:none;border-top:1px solid #e2e8f0;margin:20px 0;">\n`;
    ta.value = ta.value.substring(0, s) + html + ta.value.substring(s);
    ta.focus();
}
...
function livePreview() {
    ...
    frame.srcdoc = html || '<p style="color:#94a3b8;text-align:center;padding:2rem;font-family:sans-serif">No content yet.</p>';
}
